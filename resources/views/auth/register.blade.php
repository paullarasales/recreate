        ...
        <div class="w-9/12 h-5/6 shadow-md overflow-hidden sm:rounded-lg flex">
            ...
            <div class="flex items-center justify-center w-1/2">
                <form method="POST" action="{{ route('register') }}" class="flex flex-col items-start justify-start w-96 p-4">
                    ...
                    <x-text-input id="email" class="outline-none border-white shadow w-96" type="email" name="email" :value="old('email')" required autocomplete="username" />
                    <x-input-error :messages="$errors->get('email')" class="mt-2" />

                    <!-- Password -->
                    <x-input-label for="password" :value="__('Password')" />
                    <x-text-input id="password" class="outline-none border-white shadow w-96" type="password" name="password" required autocomplete="new-password" />
                    <x-input-error :messages="$errors->get('password')" class="mt-2" />

                    <!-- Confirm Password -->
                    <x-input-label for="password_confirmation" :value="__('Confirm Password')" />
                    <x-text-input id="password_confirmation" class="outline-none border-white shadow w-96" type="password" name="password_confirmation" required autocomplete="new-password" />
